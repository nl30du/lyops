...
class ResultsCollector(default.CallbackModule):
    ...
    def v2_runner_on_failed(self, result, ignore_errors=False):
        ...
        if ignore_errors:
            ...
            if self.host_ok.get(result._host.get_name(), False):
                self.host_ok.pop(result._host.get_name())
...
class ResultsCollectorForhoc(CallbackBase):
    ...
    def v2_runner_on_unreachable(self, result):
        # print dir(result)
        # print result._result
        printinfo(self.module_name, result, 'unreachable')
        self.host_unreachable[result._host.get_name()] = result

    def v2_runner_on_ok(self, result):
        # print dir(result)
        # print result._result
        printinfo(self.module_name, result, 'ok')
        self.host_ok[result._host.get_name()] = result

    def v2_runner_on_failed(self, result, *args, **kwargs):
        # print result._result
        printinfo(self.module_name, result, 'failed')
        self.host_failed[result._host.get_name()] = result
...
class AnsibleAPI(object):
    ...
    def run(self, host_list, module_name, module_args):
        ...
        # actually run it
        tqm = None
        self.callback = ResultsCollectorForhoc(module_name)
        try:
    ...
    def run_playbook(self, host_list=None, role_name=None, role_uuid=None, temp_param=None):
        """
        run ansible palybook
        """
        try:
            self.callback = ResultsCollector()
            filenames = ['' + '/etc/ansible/playbooks/test.yml']  # playbook的路径
    ...
    def get_result(self):
        ...
        for host, result in self.callback.host_failed.items():
            self.results_raw['failed'][host] = [result._result.get('msg') or result._result, str(result._task)]
...
if __name__ == '__main__':
    ...
    aa = AnsibleAPI(resource)
    aa.run(['192.168.146.131'], 'command', ['ifconfig'])
